  ...
  if($check_cart->rowCount() > 0){
     ...
     if($address == ''){
      ...
      $insert_notif->execute([$user_id, $notif, $status, $from]);
        ...
        $insert_order = $conn->prepare("INSERT INTO `orders`(user_id, admin_id, order_number, name, number, email, method, address, total_products, total_price, placed_on, cancel_reason) VALUES(?,NULL,?,?,?,?,?,?,?,?,?,NULL)");
